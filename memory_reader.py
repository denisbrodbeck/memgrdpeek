...
EAGER_READ = KEY_SIZE * 5

def get_maps(pid, mode='w'):
    maps = []
    with open('/proc/{0}/maps'.format(pid), 'r') as file:
        for line in file:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Recover protected key from memory of memguarded process')
    parser.add_argument('--pid', required=True, default=None,
                        help='Process id of the app')
